...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($username) && !empty($password)) {
        // Prepare the SQL query to select the admin user
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                if (password_verify($password, $hashed_password)) {
                    ...
                    logActivity($conn, 'Admin Login', "Admin ID $id logged in successfully.");
                    ...
                    logActivity($conn, 'Failed Admin Login', "Admin with username '$username' failed to log in.");
                ...
                // Log unsuccessful login attempt
                logActivity($conn, 'Failed Admin Login', "No user found with username '$username'.");
